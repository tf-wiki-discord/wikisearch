...
const client = new Client({
    intents: [Intents.FLAGS.GUILDS, Intents.FLAGS.GUILD_MESSAGES, Intents.FLAGS.GUILD_MEMBERS, Intents.FLAGS.MANAGE_MESSAGES]
});
  ...
  if(/youtube\.com\/watch\?v=/.test(msg.content)) {
	  ...
	  getYTtitle(ytslug).then(ytdata => {
		...
		if(ytdata.items[0].snippet.channelTitle.includes("JobbytheHong")) {
			...
			.setTitle("No Jobby-posting! Instead let\'s talk about <i>ANYTHING</i> else.")
			.setAuthor('Radbot')
			.setDescription('')
			.setThumbnail()
